"""
================================================================================
FINE-TUNE BioViL-T (day du) tren set_A   [Label Smoothing]
================================================================================
Giong testf5.py NHUNG:
  - Focal gamma = 2.0  (testf5 dung 5.0)
  - THEM label_smoothing vao Focal Loss (qua CrossEntropyLoss ben trong)

Muc dich: xem label smoothing co giup AUC/tong quat hoa tot hon khong.
Split GIU Y HET (GroupShuffleSplit seed 42) -> cung tap test 339 anh.

Output tu dong luu vao chinh folder nay (finetunelabelsmoothing/).

CHAY:
  python "finetunelabelsmoothing/finetune_biovilt_labelsmoothing.py" --smoke
  python "finetunelabelsmoothing/finetune_biovilt_labelsmoothing.py"
================================================================================
"""
import os
import re
...
OUTPUT_DIR    = os.path.dirname(os.path.abspath(__file__))   # luu ngay trong folder nay
...
CROP           = 448
PATIENCE       = 7
FOCAL_GAMMA     = 2.0    # <-- doi tu 5.0 (testf5) ve 2.0
LABEL_SMOOTHING = 0.1    # <-- THEM label smoothing (thu 0.05 / 0.1)
...
criterion = FocalLoss(alpha=weights_tensor, gamma=FOCAL_GAMMA, label_smoothing=LABEL_SMOOTHING)
...
CKPT = os.path.join(OUTPUT_DIR, "best_model_biovilt_ls.pth")
...
if not args.smoke:
    ...
    fig.suptitle("BioViL-T + Label Smoothing — Benh Nghe Nghiep", fontsize=14, fontweight="bold")
    ...
    plt.tight_layout()
    out_png = os.path.join(OUTPUT_DIR, "evaluation_biovilt_ls.png")
    plt.savefig(out_png, dpi=150, bbox_inches="tight"); print(f"\nSaved -> {out_png}")
    import csv as _csv
    with open(os.path.join(OUTPUT_DIR, "confusion_matrix_biovilt_ls.csv"), "w", newline="", encoding="utf-8") as f:
    ...
    print("Saved -> confusion_matrix_biovilt_ls.csv")

print("\n" + "=" * 80)
print(f"DONE — BioViL-T + LabelSmoothing({LABEL_SMOOTHING}) | AUC={roc_auc:.4f} | Sens={sens:.3f} | Spec={spec:.3f}")
print("=" * 80)
