"""
================================================================================
ABLATION LOSS #2 — Weighted CrossEntropy   [Finetune WeightedCE]
================================================================================
Thi nghiem HAM LOSS (yeu cau thay). GIU NGUYEN Finetune 4 (BioViL-T),
CHI DOI ham loss -> Weighted CrossEntropy (CE + trong so lop).

Y nghia: tach rieng tac dong cua TRONG SO LOP.
  - CrossEntropy      : khong trong so           (da chay -> AUC 0.913)
  - Weighted CE       : CE + class weight  <-- file nay  (= Focal gamma=0)
  - Focal gamma=2     : CE + class weight + focus ca kho (Finetune 4 -> 0.902)

CHI KHAC DUNG 1 DONG:
    criterion = nn.CrossEntropyLoss(weight=weights_tensor)
Con lai giong het Finetune 4 (backbone, data, split seed 42, sampler, 2-stage, Youden).

Output (vao Finetune WeightedCE/):
  - best_model_wce.pth, evaluation_wce.png, confusion_matrix_wce.csv, train_log.txt

CHAY:
  python "Finetune WeightedCE/finetune_biovilt_wce.py" --smoke
  python "Finetune WeightedCE/finetune_biovilt_wce.py"
================================================================================
"""
import os
import re
...
OUTPUT_DIR    = r"e:\Hoc hanh\ĐÒ ÁN\Data DGCNN\Finetune WeightedCE"
...
print("=" * 80)
print(f"ABLATION LOSS #2 — BioViL-T + Weighted CrossEntropy  {'[SMOKE]' if args.smoke else ''}")
print("=" * 80)
print(f"LOSS = nn.CrossEntropyLoss(weight=class_weights)  (CE + trong so lop)")
...
# ================================================================================
# >>> CHO DUY NHAT KHAC: HAM LOSS = Weighted CrossEntropy <<<
# Finetune 4:            FocalLoss(alpha=weights_tensor, gamma=2.0)
# Finetune CrossEntropy: nn.CrossEntropyLoss()
# Finetune WeightedCE:   nn.CrossEntropyLoss(weight=weights_tensor)   <-- day
# ================================================================================
criterion = nn.CrossEntropyLoss(weight=weights_tensor)
...
CKPT = os.path.join(OUTPUT_DIR, "best_model_wce.pth")
...
print("\n" + "=" * 80)
print("TEST (Youden's J) — LOSS = Weighted CrossEntropy")
print("=" * 80)
...
if not args.smoke:
    ...
    fig.suptitle("BioViL-T + Weighted CrossEntropy — Benh Nghe Nghiep", fontsize=14, fontweight="bold")
    ...
    plt.tight_layout()
    out_png = os.path.join(OUTPUT_DIR, "evaluation_wce.png")
    plt.savefig(out_png, dpi=150, bbox_inches="tight"); print(f"\nSaved -> {out_png}")
    import csv as _csv
    with open(os.path.join(OUTPUT_DIR, "confusion_matrix_wce.csv"), "w", newline="", encoding="utf-8") as f:
    ...
    print("Saved -> confusion_matrix_wce.csv")

print("\n" + "=" * 80)
print(f"DONE — BioViL-T + Weighted CE | AUC={roc_auc:.4f} | Sens={sens:.3f} | Spec={spec:.3f}")
print("=" * 80)
