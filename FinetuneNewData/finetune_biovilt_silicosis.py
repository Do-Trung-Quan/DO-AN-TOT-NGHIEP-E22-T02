...
ABLATION HAM LOSS (--loss), giong cach lam o Finetune CrossEntropy:
  --loss focal (mac dinh) : FocalLoss(alpha=class weights, gamma=2)
  --loss ce               : nn.CrossEntropyLoss()  (mac dinh, KHONG class weight)
CHI KHAC DUNG 1 DONG criterion, moi thu con lai (data, split seed 42, sampler,
2-stage, augmentation, LR, Youden's J) GIU NGUYEN -> so sanh cong bang.

Output (vao Finetune Silicosis/), TAG = "" voi focal / "_ce" voi crossentropy:
  - best_model_biovilt_silicosis{TAG}.pth
  - evaluation_biovilt_silicosis{TAG}.png
  - confusion_matrix_biovilt_silicosis{TAG}.csv
  - test_predictions_silicosis{TAG}.csv
  - train_log_silicosis{TAG}.txt
=> chay ca 2 loss khong de len ket qua cua nhau.

CHAY:
  # test nhanh truoc (2 epoch, it anh):
  python "Finetune Silicosis/finetune_biovilt_silicosis.py" --smoke
  # chay that (focal, nhu cu):
  python "Finetune Silicosis/finetune_biovilt_silicosis.py"
  # chay ablation CrossEntropy:
  python "Finetune Silicosis/finetune_biovilt_silicosis.py" --loss ce
================================================================================
"""
import os
import sys
...
parser.add_argument("--loss", choices=["focal", "ce"], default="focal",
                    help="focal = FocalLoss(gamma=2)+class weight (mac dinh); "
                         "ce = nn.CrossEntropyLoss() thuan (ablation)")
...
# hau to file output theo ham loss -> chay focal va ce khong ghi de len nhau
TAG           = "" if args.loss == "focal" else f"_{args.loss}"
LOSS_DESC     = ("FocalLoss(gamma=2) + class weights" if args.loss == "focal"
                 else "nn.CrossEntropyLoss() (mac dinh, KHONG class weight)")
...
_logname = (f"train_log_silicosis{TAG}_smoke.txt" if args.smoke
            else f"train_log_silicosis{TAG}.txt")
...
# ================================================================================
# >>> CHO DUY NHAT KHAC GIUA 2 THI NGHIEM: HAM LOSS <<<
#   --loss focal : FocalLoss(alpha=class weights, gamma=2)
#   --loss ce    : nn.CrossEntropyLoss()  (mac dinh, KHONG class weight, KHONG focal)
# Sampler / split / augmentation / LR / 2-stage GIU NGUYEN de so sanh cong bang.
# ================================================================================
if args.loss == "focal":
...
CKPT = os.path.join(OUTPUT_DIR, f"best_model_biovilt_silicosis{TAG}.pth")
...
print("\n" + "=" * 80)
print(f"TEST (Youden's J) — LOSS = {args.loss}")
print("=" * 80)
...
if not args.smoke:
    pd.DataFrame({"file_name": test_df["file_name"].values,
                 ...
                 ).to_csv(os.path.join(OUTPUT_DIR, f"test_predictions_silicosis{TAG}.csv"),
                          index=False, encoding="utf-8")

    thr_list = np.arange(0.05, 0.95, 0.05)
    f1s = [f1_score(y_true, (y_proba >= t).astype(int), zero_division=0) for t in thr_list]
    fig, ax = plt.subplots(2, 2, figsize=(14, 10))
    _loss_title = "Focal loss" if args.loss == "focal" else "CrossEntropy loss"
    fig.suptitle(f"BioViL-T fine-tune ({_loss_title}) — Silicosis vs Normal",
    ...
    plt.tight_layout()
    out_png = os.path.join(OUTPUT_DIR, f"evaluation_biovilt_silicosis{TAG}.png")
    plt.savefig(out_png, dpi=150, bbox_inches="tight"); print(f"\nSaved -> {out_png}")

    import csv as _csv
    _cm_csv = f"confusion_matrix_biovilt_silicosis{TAG}.csv"
    with open(os.path.join(OUTPUT_DIR, _cm_csv), "w",
...
print("\n" + "=" * 80)
print(f"DONE — BioViL-T silicosis [loss={args.loss}] | AUC={roc_auc:.4f} | Sens={sens:.3f} | Spec={spec:.3f}")
print("=" * 80)
