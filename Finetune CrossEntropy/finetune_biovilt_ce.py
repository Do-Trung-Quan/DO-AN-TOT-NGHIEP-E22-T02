"""
================================================================================
ABLATION LOSS #1 — CrossEntropy (loss mac dinh)   [Finetune CrossEntropy]
================================================================================
Muc dich: thi nghiem HAM LOSS theo yeu cau cua thay.
GIU NGUYEN mọi thu giong Finetune 4 (BioViL-T), CHI DOI 1 thu: ham loss.

  - Finetune 4          : BioViL-T + FocalLoss(gamma=2) + class weight  -> AUC 0.902
  - Finetune CrossEntropy: BioViL-T + CrossEntropyLoss (mac dinh)        <-- file nay

CHI KHAC DUNG 1 DONG:
    criterion = nn.CrossEntropyLoss()      # thay vi FocalLoss(...)
Con lai: backbone BioViL-T, data set_A, split seed 42, sampler, 2-stage,
augmentation, LR, Youden's J -> GIONG HET Finetune 4 (de so sanh cong bang).

Output (vao Finetune CrossEntropy/):
  - best_model_ce.pth, evaluation_ce.png, confusion_matrix_ce.csv, train_log.txt

CHAY:
  python "Finetune CrossEntropy/finetune_biovilt_ce.py" --smoke   # test nhanh
  python "Finetune CrossEntropy/finetune_biovilt_ce.py"           # chay that
================================================================================
"""
import os
import re
...
OUTPUT_DIR    = r"e:\Hoc hanh\ĐÒ ÁN\Data DGCNN\Finetune CrossEntropy"
...
print("=" * 80)
print(f"ABLATION LOSS #1 — BioViL-T + CrossEntropy  {'[SMOKE]' if args.smoke else ''}")
print("=" * 80)
print(f"LOSS = nn.CrossEntropyLoss()  (mac dinh, KHONG class weight, KHONG focal)")
...
# ================================================================================
# 2) TRANSFORMS  (giong Finetune 4: pipeline BioViL-T)
# ================================================================================
...
# ================================================================================
# 3) SAMPLER + LOADERS  (GIU NGUYEN giong Finetune 4 -> chi loss thay doi)
# ================================================================================
...
print(f"  Counts {dict(counts)} | sampler GIU NGUYEN (chi loss doi)")
...
# ================================================================================
# >>> CHO DUY NHAT KHAC Finetune 4: HAM LOSS <<<
# Finetune 4:            criterion = FocalLoss(alpha=weights_tensor, gamma=2.0)
# Finetune CrossEntropy: criterion = nn.CrossEntropyLoss()   (mac dinh)
# ================================================================================
...
CKPT = os.path.join(OUTPUT_DIR, "best_model_ce.pth")
...
print("\n" + "=" * 80)
print("TEST (Youden's J) — LOSS = CrossEntropy")
print("=" * 80)
...
if not args.smoke:
    ...
    fig.suptitle("BioViL-T + CrossEntropy loss — Benh Nghe Nghiep", fontsize=14, fontweight="bold")
    ...
    plt.tight_layout()
    out_png = os.path.join(OUTPUT_DIR, "evaluation_ce.png")
    plt.savefig(out_png, dpi=150, bbox_inches="tight"); print(f"\nSaved -> {out_png}")
    import csv as _csv
    with open(os.path.join(OUTPUT_DIR, "confusion_matrix_ce.csv"), "w", newline="", encoding="utf-8") as f:
    ...
    print("Saved -> confusion_matrix_ce.csv")

print("\n" + "=" * 80)
print(f"DONE — BioViL-T + CrossEntropy | AUC={roc_auc:.4f} | Sens={sens:.3f} | Spec={spec:.3f}")
print("=" * 80)
