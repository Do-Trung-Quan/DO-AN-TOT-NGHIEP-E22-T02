...
OUTPUT_DIR    = r"e:\Hoc hanh\ĐÒ ÁN\Data DGCNN\Finetune 4"
...
CKPT = os.path.join(OUTPUT_DIR, "best_model_biovilt_finetuned.pth")
...
if not args.smoke:
    ...
    plt.tight_layout()
    out_png = os.path.join(OUTPUT_DIR, "evaluation_biovilt_finetune.png")
    plt.savefig(out_png, dpi=150, bbox_inches="tight"); print(f"\nSaved -> {out_png}")
    import csv as _csv
    with open(os.path.join(OUTPUT_DIR, "confusion_matrix_biovilt_finetune.csv"), "w", newline="", encoding="utf-8") as f:
